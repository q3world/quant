        ...
        def df_compute(symbol, interval, df):
            ...
            for time_period in time_period_list:
                if time_period < len(df):
                    ...
                    price = c_values[-1]
                    value = ma[-1]
                    #print('{:>10} {:>10.3f} {:>10} {:>10d} {:>10.4f} {:>10.4%}'.format(symbol, price, interval, time_period, value, price/value-1) )
                    if price > value and price < value*1.01 and ma[-1] > ma[-2] and ma[-2] > ma[-3]:
                        flag = 0
                        for i in range(time_period//2 + 1):
                            if l_values[-i] > ma[-i]:
                                flag = 1
                                break
...
class market:
    ...
    def get_quote(symbol):
        if not 'Ak'in market._quotes:
            market._quotes['Ak'] = quote_Ak()
        
        return market._quotes['Ak']
    ...
    def get_strategy():
        if not 'MA'in market._strategies:
            market._strategies['MA'] = strategy_MA()
